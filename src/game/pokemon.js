// ═══════════════════════════════════════════════
//  DADOS DE POKÉMON — 4 REGIÕES (80+ Pokémon)
// ═══════════════════════════════════════════════
...
// Pesos de raridade (quanto maior, mais frequente)
const RARITY_WEIGHT = {
  comum:    50,
  incomum:  30,
  raro:     15,
...
function getRandomWildPokemon(region, trainerLevel = 1) {
  ...
  for (const p of pool) {
    ...
    for (let i = 0; i < weight * 10; i++) weighted.push(p);
  }

  const base = weighted[Math.floor(Math.random() * weighted.length)];
  const wildLevel = Math.max(1, Math.min(
    trainerLevel + Math.floor(Math.random() * 5) - 2,
    100
  ));
...
function getStartersByRegion(region) {
  ...
  return (starters[region] || starters.kanto)
    .map(id => buildPokemon(POKEMON_DATA[id], 5));
}
